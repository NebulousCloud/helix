...
function PANEL:OnDrop(bDragging, inventoryPanel, inventory, gridX, gridY, centerX, centerY)
	...
	elseif (!inventoryPanel:IsEmpty(centerX, centerY, self)) then
		if (inventoryPanel.slots[centerX] and inventoryPanel.slots[centerX][centerY]) then
			...
			if (IsValid(itemPanel) and combineFunction) then
				local data = {itemPanel.itemTable.id}

				item.player = LocalPlayer()
					if (combineFunction.OnCanRun and combineFunction.OnCanRun(item, data) != false) then
						InventoryAction("combine", item.id, inventoryPanel.invID, data)
					end
				item.player = nil
			end
		end
